...
@ManagedBean
@ViewScoped
public class ValidacionIngresoBean implements Serializable {
	private static final long serialVersionUID = 1L;

	private List<OpeDetalleTransaccion> listaNoValidadasI; // lista para guardar
															// el resultado de
															// la consulta,
															// pendientes por
															// validar en
															// ingreso
	private List<OpeDetalleTransaccion> listaNoValidadasS; // lista para guardar
															// el resultado de
															// la consulta,
															// pendientes por
															// validar en salida
	...
	public void guardarTransaccion() {
		...
		for (OpeDetalleTransaccion item : listaNoValidadasI) {
	...
	public void consultaNoValidasI() { // consultas las fundas no validas en su
										// ingreso a la boveda de tulas en
										// transito
		setListaNoValidadasI(new ArrayList<>());
		setListaNoValidadasI(servicioTransaccion.noValidadasI(idCiudad, transaccion.getFechaOperacion()));
		consultaListaReasignacion();

	}

	public void consultaNoValidasS() { // consultas las fundas no validas en su
										// salida de la boveda de tulas en
										// transito
	...
	public void guardarValidacion() { // guarda las validaciones qeu se hacen
										// medinte la lectura con el escanner de
										// mano
		...
		for (OpeDetalleTransaccion selloEsca : sellosEscaneados) {
			for (OpeDetalleTransaccion selloPend : listaNoValidadasI) {
				if (selloEsca.getSello().equals(selloPend.getSello())) {
					...
					servicioValidacion.create(validacion);// insertar fila en
															// tabla
															// detallevalidado
					servicioDetalleTransaccion.actualizaValidado(selloPend.getIdDetalleTransaccion());// setear
																										// el
																										// true
		...
		// listaNumerosRecibos = contiene los recibos de remesa que si fueron
		// escaneados
	...
	public void guardarValidacionManual() { // guarad las validaciones cuando se
											// hcen de forma manual
		AdmUsuario us = (AdmUsuario) FacesContext.getCurrentInstance().getExternalContext().getSessionMap()
				.get("usuario"); // obtengo el usuario logueado
	...
	public List<OpeDetalleTransaccion> getListaNoValidadasI() {
		return listaNoValidadasI;
	}

	public void setListaNoValidadasI(List<OpeDetalleTransaccion> listaNoValidadasI) {
		this.listaNoValidadasI = listaNoValidadasI;
	}
