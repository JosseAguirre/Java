...
public class AdmUsuarioBean implements  Serializable {
	...
	public void guardar() {
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Aviso",
					"Se registró el Usuario: " + admusuario.getUsuario()));
			consultaListaUsuarios();
			cancelar();
		} catch (Exception e) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, "Aviso", "Se ha producido un error al guardar"));
		}
	...
	public void eliminar() {
		if (usuarioseleccionado == null) {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Aviso", "Datos Eliminado Correctamente "));
			consultaListaUsuarios();
			cancelar();
		}
	}
	...
	public void actualizar() {
		try {
			serviciousuario.update(admusuario);
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Aviso", "Regitro actualizado con exito"));

		} catch (Exception e) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Aviso", "Se produjo un error al actualizar :" + e));
		}
	}
	...
	public void consultaListaCombos() {
		listaempleado = new ArrayList<>(); // Creo una lista para mostar todo los empleado en un datatable
	...
	public void consultaListaAgencia() {
		...
		setListaAgencia(servicioAgencia.buscaAgenciaCiudad(ciudad, idCliente)); 
		consultaAgenciasAsignadas();
	}
