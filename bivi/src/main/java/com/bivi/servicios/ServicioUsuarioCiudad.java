...
public class ServicioUsuarioCiudad {
	...
	public List<AdmUsuarioCiudad> buscaCiudadesAsiganadas(AdmUsuario us) {
		...
		Query q = em.createNativeQuery("select uc.id_usuario_ciudad, dc.descripcion, dc.id_detalle_catalogo  from bivi.adm_usuario_ciudad as uc "+
		"inner join bivi.adm_usuario as u on u.id_usuario = uc.id_usuario "+
		"inner join bivi.adm_detalle_catalogo as dc on dc.id_detalle_catalogo = uc.id_ciudad_dc "+
		...
		for(Object[] row : rows){
			...
			d.setDescripcion(row[1].toString());
			d.setIdDetalleCatalogo(Integer.parseInt(row[2].toString()));
			
			e.setIdDetalleCatalogo(d);
